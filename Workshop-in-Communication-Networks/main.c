...
//      recvbuf[i] = (int)(10 * i + rank);
////      printf("%d ", sendbuf[i]);
//    }
////    printf ("\n");
//
//    // Run all-reduce (RS + AG). recvbuf will hold the full reduced array.
//    if (pg_all_reduce(recvbuf, recvbuf, (int)count,
...
////ours check logic all_reduce - 1 size - sendbuf == recvbuf
//int main(int argc, char const *argv[]) {
//  if (argc != 2) {
//    fprintf(stderr, "Usage: %s \"server1 server2 ... servern\"\n", argv[0]);
//    return EXIT_FAILURE;
//  }
//
//  pg_handle* handle = NULL;
//  if (connect_process_group(argv[1], (void**)&handle)) {
//    close_pg_handle(handle);
//    return EXIT_FAILURE;
//  }
//
...
//
//
//  size_t count = 16;   // up to 3<<20 == buffer_size
//
//  // Initialize input: value(i, rank) = 10*i + rank
//  for (size_t i = 0; i < count; ++i) {
//    recvbuf[i] = (int)(10 * i + rank);
...
//    printf ("\n");
//
//    printf("======== starting all reduce ========== \n");
//  // Run all-reduce (RS + AG). recvbuf will hold the full reduced array.
//  if (pg_all_reduce(recvbuf, recvbuf, (int)count, DT_INT32,
//                    OP_SUM, handle)) {
//    fprintf(stderr, "[Rank %d] pg_all_reduce returned error\n", rank);
////    free(sendbuf);
//    free(recvbuf);
